...
public class NavMeshSurfaceAgentTests
{
    ...
    {
        var surface = UnityEngine.GameObject.CreatePrimitive(PrimitiveType.Plane).AddComponent<NavMeshSurface>();
        surface.BuildNavMesh();

        var agent = new UnityEngine.GameObject("Agent").AddComponent<NavMeshAgent>();
        Assert.AreEqual(surface, agent.navMeshOwner);
        Assert.IsTrue(agent.isOnNavMesh);

        UnityEngine.GameObject.DestroyImmediate(agent.gameObject);
