...
{
    public UnityEngine.GameObject plane1, plane2;
    ...
    {
        plane1 = UnityEngine.GameObject.CreatePrimitive(PrimitiveType.Plane);
        plane2 = UnityEngine.GameObject.CreatePrimitive(PrimitiveType.Plane);
        plane1.transform.position = 11.0f * Vector3.right;

        surface = new UnityEngine.GameObject().AddComponent<NavMeshSurface>();
        surface.BuildNavMesh();

        Assert.IsFalse(HasPathConnecting(plane1, plane2));
        Assert.IsFalse(HasPathConnecting(plane2, plane1));

        link = new UnityEngine.GameObject().AddComponent<NavMeshLink>();
    ...
    {
        UnityEngine.GameObject.DestroyImmediate(surface.gameObject);
        UnityEngine.GameObject.DestroyImmediate(link.gameObject);
        UnityEngine.GameObject.DestroyImmediate(plane1);
        UnityEngine.GameObject.DestroyImmediate(plane2);
    }
    ...
    public static bool HasPathConnecting(UnityEngine.GameObject a, UnityEngine.GameObject b, int areaMask = NavMesh.AllAreas, int agentTypeID = 0)
    {
    ...
    public static bool HasPathConnectingViaPoint(UnityEngine.GameObject a, UnityEngine.GameObject b, Vector3 point, int areaMask = NavMesh.AllAreas, int agentTypeID = 0)
    {
        var path = new NavMeshPath();
        var filter = new NavMeshQueryFilter();
        filter.areaMask = areaMask;
